    ...
    public class SqlDatabaseManager : IDatabaseManager
    {
        ...
        private TableModel ConvertTabelToModel(Table item)
        {
            var  table = new TableModel()
            {
                Name = item.Name,
                IdDb = item.ID,
                ColumnList = GetColumns(item.Name),
        ...
        public List<TableModel> GetViews()
        {
            if (_selectedDatabase != null)
            {
                ...
                foreach (Table item in _selectedDatabase.Views)
                {
                    if (item.IsSystemObject == false)
                    {

                        views.Add(ConvertTabelToModel(item));
                    }
        ...
        //}
        public List<IColumnModel> GetColumns(string tabelname)
        {
